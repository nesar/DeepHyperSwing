...
def execute_search(evaluator):
    ...
    search = CBO(problem, evaluator, surrogate_model="GP", moo_scalarization_strategy="rChebyshev", initial_points=[problem.default_configuration], log_dir="cbo-moo-results", random_state=42)
...
def plot_sum_up(name, init_duration):
    ...
    fig.tight_layout()
    rint = np.random.randint(1000)
    plt.savefig(f"plots/{name}"+str(rint)+".png", bbox_inches="tight")
